...
func main() {
	...
	go func() {
		defer wg.Done()
		oprt.Operator(startTime, endTime, diff, speed, solarOutput, windSpeed, startHour)
	}()
	go func() {
		defer wg.Done()
		prdc.AllProducers(startTime, endTime, diff, speed, interval, tokenLife, solarOutput, windSpeed, seaWindSpeed, startHour)
	}()
	go func() {
		defer wg.Done()
