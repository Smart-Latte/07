...
func Produce(contract *client.Contract, username string, lat float64, lon float64, category string, output float64, outputList [dayNum][hourNum]float64, seed int64) {
	...
	timer := time.NewTimer((time.Duration(waitNano) * time.Nanosecond + time.Duration(5 + int64(wait) + time.Now().UnixNano() - Diff - StartTime) * time.Nanosecond) / time.Duration(Speed))
