...
func AllProducers(start int64, end int64, difference int64, mySpeed int64, auctionInterval int64, life int64, sOutput [dayNum][hourNum]float64, wOutput [dayNum][hourNum]float64, 
	...
	for i := 0; i < dayNum; i++ {
		for j := 0; j < hourNum; j++ {
			...
			var seaWind float64
			var dWind float64
			if cutIn <= SeaWindOutput[i][j] {
				seaWind = realWindOut * math.Pow((SeaWindOutput[i][j] / rating), 3) * 4
				dWind = dummyWind * math.Pow((WindOutput[i][j] / rating), 3) * 4
			}
			out := solar + dSolar + seaWind + dWind
			if (i == 0 && j < 18) {
				thermalOutput[i][j] = 2000000 - out
			} else {
				thermalOutput[i][j] = 1200000 - out
			}
	...
	go func() {
		defer wg.Done()
		SeaWindProducer(contract, "real-wind-producer0", 40.2160279724715, 140.002846271612, "wind", realWindOut, rating, cutIn, SeaWindOutput, 1)
	}()
	go func() {
		defer wg.Done()
		SeaWindProducer(contract, "real-wind-producer1", 40.2095028757269, 139.997337258476, "wind", realWindOut, rating, cutIn, SeaWindOutput, 2)
	}()
	go func() {
		defer wg.Done()
		SeaWindProducer(contract, "real-wind-producer2", 40.2021377588529, 140.068615482843, "wind", realWindOut, rating, cutIn, SeaWindOutput, 3)
	}()
	go func() {
		defer wg.Done()
	...
	for i := 0; i < 4; i++ { // 4
		...
		go func(n int) {
			defer wg.Done()
			DummyWindProducer(contract, fmt.Sprintf("windProducerGroup%d", n), 40.17463042136363, 40.2330209148308, 139.992165531859, 140.068615482843, "wind", dummyWind, rating, cutIn, WindOutput, int64(n + 1000))
