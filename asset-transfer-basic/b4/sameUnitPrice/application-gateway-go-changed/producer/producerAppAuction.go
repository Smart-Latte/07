...
func Auction(contract *client.Contract, energy Energy) {
	timer := time.NewTimer(time.Duration(Interval * 60 - ((time.Now().UnixNano() - Diff - StartTime) * Speed + StartTime - energy.GeneratedTime)) * time.Nanosecond / time.Duration(Speed))
